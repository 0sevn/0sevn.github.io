/**
 * Cloudify.js (c) 2016-2019 @ Cong Min
 * MIT License - https://github.com/cong-min/Cloudify
 */
...
class Cloudify {
    ...
    constructor(container = document.body, texts, options) {
        ...
        self.texts = texts || [];
        self.config = { ...Cloudify._defaultConfig, ...options || {} };
        ...
        self.maxSpeed = Cloudify._getMaxSpeed(self.config.maxSpeed); // rolling max speed
        self.initSpeed = Cloudify._getInitSpeed(self.config.initSpeed); // rolling init speed
        ...
        Cloudify.list.push({ el: self.$el, container, instance: self });
    }
    ...
    static _defaultConfig = {
        ...
        containerClass: 'Cloudify',
        itemClass: 'Cloudify--item',
        useHTML: false,
    };
    ...
    _init() {
        ...
        if (!isTouchDevice || isTouchDevice.matches) {
            // mouseover
            Cloudify._on(self.$el, 'mouseover', () => { self.active = true; });
            // mouseout
            Cloudify._on(self.$el, 'mouseout', () => { self.active = false; });
            // mousemove
            Cloudify._on(self.keep ? window : self.$el, 'mousemove', (ev) => {
                ev = ev || window.event;
    ...
    destroy() {
        const self = this;
        self.interval = null;
        // clear in Cloudify.list
        const index = Cloudify.list.findIndex(e => e.el === self.$el);
        if (index !== -1) Cloudify.list.splice(index, 1);
        // clear element
...
export default (els, texts, options) => {
    ...
    els.forEach(el => {
        if (el) {
            instances.push(new Cloudify(el, texts, options));
        }
    });
