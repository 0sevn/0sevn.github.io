// console.log("test: Script is loaded1", self.config.layoutMode);
...
(function (global, factory) {
    ...
    _createClass(TagCloud, [{
      ...
      function _createElment() {
        ...
        if (self.config.useContainerInlineStyles) {
          // console.log("test: UseContainerInlineStyles");
        ...
        // If grid layout, set positions once and do not animate
      if (self.config.layoutMode === 'grid') {
        
        self.items.forEach(function (item, index) {
          var position = self._computePosition(index);
      ...
      value: function _createTextItem(text) {
        ...
        if (self.config.useItemInlineStyles) {
          ...
          itemEl.style.zIndex = index + 1;
            // Set opacity to 1 only for grid layout
            if (self.config.layoutMode === 'grid') {
              itemEl.style.filter = 'alpha(opacity=100)';
              itemEl.style.opacity = 1;  // Set opacity to 1 for grid layout
            } else {
              itemEl.style.filter = 'alpha(opacity=0)';
              itemEl.style.opacity = 0;  // Default opacity for spherical layout
            }
      ...
      value: function _computePosition(index) {
        ...
        if (self.config.layoutMode === 'grid') {
          console.log("grid")
          const columns = self.config.gridColumns || 1;
          const gap = self.config.gridGap || 1;
          const containerWidth = self.$container.clientWidth || 200;
          const containerHeight = self.$container.clientHeight || 300;
          ...
          return { x, y, z: 0 };
              ...
              // console.log("this is by default spherical")
      ...
      value: function _init() {
        ...
        if (self.config.layoutMode === 'sphere') {
  ...
  TagCloud._defaultConfig = {
    radius: 300,    // rolling radius, unit `px`
    ...
    gridColumns: 1, // Number of columns in grid layout
    gridGap: 1 // Gap between grid cells
  };
  // speed value
